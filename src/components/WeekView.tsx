  ...
  return (
    <Box>
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ px: 2, py: 1.5 }}>
        ...
        {/* Goals Editor Dialog */}
        {week && <GoalsEditor open={goalsOpen} onClose={handleCloseGoals} week={week} onSave={handleSaveGoals} />}
      </Stack>
        ...
        <Grid container spacing={2} sx={{ mt: 1 }}>
          <Grid size={{ xs: 4 }}>
            ...
            <Typography sx={{ fontFamily: "monospace" }}>
              {Math.round(summary.protein)}/{week?.proteinGoal ?? "--"}g
            </Typography>
            <LinearProgress
              variant="determinate"
              value={pct(summary.protein, week?.proteinGoal)}
              sx={{
                bgcolor: "#134E4A",
                "& .MuiLinearProgress-bar": { bgcolor: "#2DD4BF" },
              }}
            />
          </Grid>
          <Grid size={{ xs: 4 }}>
            ...
            <Typography sx={{ fontFamily: "monospace" }}>
              {Math.round(summary.carbs)}/{week?.carbGoal ?? "--"}g
            </Typography>
          ...
          <Grid size={{ xs: 4 }}>
            ...
            <Typography sx={{ fontFamily: "monospace" }}>
              {Math.round(summary.fat)}/{week?.fatGoal ?? "--"}g
            </Typography>
            <LinearProgress
              variant="determinate"
              value={pct(summary.fat, week?.fatGoal)}
              sx={{
                bgcolor: "#0E3A5E",
      ...
      {/* Day Selector Tabs */}
      <Tabs value={selectedDayIndex} onChange={(_, idx) => setSelectedDayIndex(idx)} variant="scrollable" scrollButtons="auto" sx={{ px: 1, mt: 2 }}>
      ...
      {/* Daily Log Panel */}
      <Box sx={{ mt: 2 }}>{week && <DailyLog weekId={week.id} date={weekDates[selectedDayIndex]} />}</Box>
    </Box>
  );
};
