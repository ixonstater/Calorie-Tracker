...
const DailyLog: React.FC<DailyLogProps> = ({ weekId, date }) => {
  ...
  const logs = useLiveQuery(
    () => db.logs.where({ weekId, date }).sortBy("timestamp"),
    [weekId, date],
  );
  ...
  return (
    <Box>
      {/* Daily Totals Row */}
      <Paper
        sx={{ p: 1.5, mb: 2, display: "flex", gap: 3, alignItems: "center" }}
      >
        <Typography fontWeight={700} sx={{ fontFamily: "monospace" }}>
          {`Total: ${Math.round(total.calories)} kcal`}
        </Typography>
        <Chip
          label={`P: ${Math.round(total.protein)}g`}
          size="small"
          sx={macroColors.protein}
        />
        <Chip
          label={`C: ${Math.round(total.carbs)}g`}
          size="small"
          sx={macroColors.carbs}
        />
        <Chip
          label={`F: ${Math.round(total.fat)}g`}
          size="small"
          sx={macroColors.fat}
        />
      </Paper>
              ...
              <React.Fragment key={log.id}>
                ...
                >
                  <ListItemText
                    primary={
                      <Typography
                        fontWeight={deleted ? 400 : 600}
                        color={deleted ? "text.secondary" : undefined}
                      >
                    ...
                    secondary={
                      deleted ? (
                        ...
                        <Typography color="text.secondary">
                          {`${log.servings} serving${log.servings !== 1 ? "s" : ""} · ${Math.round(log.servings * food!.calories)} kcal · P ${Math.round(log.servings * food!.protein)}g · C ${Math.round(log.servings * food!.carbs)}g · F ${Math.round(log.servings * food!.fat)}g`}
                        </Typography>
                      )
                    }
                  />
